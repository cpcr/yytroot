	...
	Time.Run = function() {
		...
		Time.goHook = function() {
			if (!$(this).hasClass('noNum')) {
				...
				}else if(isHasAppointmentReg && hookDays <= Time.D){//预约挂号小于等于今天的日期都判断为下个月的日期
					//小于今天的日期  可判断为下个月的日期
					//判断本月是否为12月 若是 则年要加1
					if(Time.M == 11){
...
function loadDoctorList(hookDate){
	...
	//加载当天的号源信息
	var url = appPath +  "/mobileApp/register/doctor/queryDoctorList?timeTemp=" + timeTemp;
...
function successExe(data){
	    	...
	    	if(doctors.length > 0){
	    		for(var i = 0 ; i < doctors.length ; i++){
					...
					if(doctors[i].leftTotalNum >0){
		           		htmlStr += "<li onclick='goDoctorTimeList(\"" + doctors[i].doctorCode + "\",\"" + doctors[i].deptCode + "\",\""  + doctors[i].deptName + "\",\""  + doctors[i].category + "\",\""   + doctors[i].picture + "\",\""  + doctors[i].timeFlag + "\")' >";
					}else{
						//0个剩余号源 不允许点击
						htmlStr += "<li>";
					}
	...
	//20160427 zkw 修改医生列表多次显示
    doctorList.html(htmlStr);
    //doctorList.append(htmlStr);
}
 ...
 * @returns
 */
function goDoctorTimeList(doctorCode , deptCode ,deptName,category , picture,timeFlag) {
	...
	if (timeFlag != null && timeFlag != undefined && timeFlag != "" && timeFlag != "null") {//增加时段 广二新需求20160826
